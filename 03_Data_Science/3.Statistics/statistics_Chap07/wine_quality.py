...
# Read the data set into a pandas DataFrrame
...
pg = sns.PairGrid(wine_sample, hue="type", hue_order=["red", "white"], \
                  palette=dict(red="red", white="white"), hue_kws={"marker": ["o", "s"]}, \
...
# Test whetehr mean quality is different between red and white wines
...
#lm = glm(my_formual, data=wine, family=sm.families.Gaussain()).fit()
#lm = smf.glm(formula_all, data=wine_standardized, family=sm.families.Gaussain()).fit()
print(lm.summary())
print("\nQuantities you cna extract from the result:\n%s" %dir(lm))
print("\nCoefficients:\n%s" %lm.params)
...
#Fit a multivariate linear model with standardized independent variables dependt_variable = wine['quality']
dependent_variable = wine['quality']
independent_variables = wine[wine.coumns.difference(['quality', 'type', 'in_sample'])]
